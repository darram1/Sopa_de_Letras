...
public class SopaLetras {
	...
	public void colocarPalabraVertical(String palabra, Coordenada coordenada) throws TableroException{
		...
		if(palabra == null || palabra.isEmpty()) {
            ...
            if ((this.n - ( coordenada.getFila()+palabra.length() ) < 0)) {
                throw new TableroException("El tama̱o de la palabra supera el limite del tablero");
            }
            	comprobador=0;
            	for(int l=0 ; l < palabra.length() ; l++ ) {
            			if (this.tablero.get(coordenada.getFila()+l).get(coordenada.getColumna()) != '*') {
            				comprobador++;
            			}
    			}
	...
	public void colocarPalabraVerticalSentidoNormal(String palabra, Coordenada coordenada) {
		...
		for(int l=0 ; l < palabra.length() ; l++ ) {
			...
			tablero.get(coordenada.getFila()+l).set(coordenada.getColumna(),letra);
		}
	}
